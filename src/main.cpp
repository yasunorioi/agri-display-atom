// agri-display-atom — EVA/NERV dashboard (multi-house)
//
// Rendering model (Atom Display framebuffer lives in the FPGA):
//   - header/background drawn once; each panel is a small PSRAM sprite pushed
//     to just its rect only when its value changes. Flicker-free + fast.
// Multi-house: every house's data is captured from the retained agriha topics
//   into a per-house store, so switching the displayed house is instant. With
//   >1 house configured, the display rotates through them on a timer.
// TV-safe: all content inset 40 px from every edge (consumer TV overscan).
...
M5Canvas sprBig(&display);     // 254x272  (CO2 / HD)
M5Canvas sprSm(&display);      // 308x120  (humid/pres/cur/flow)
M5Canvas sprWin(&display);     // 596x88   (window bar)
...
// ---- layout (inside a 40 px TV-safe margin) ---------------------------------
#define MX      40
#define HDR_H   58
#define HERO_X  MX
#define HERO_Y  66
...
// house rotation (1 house = static; >1 = cycle every g_rotSecs)
static char g_housesStr[24] = "2";
static int  g_rotSecs   = 15;
static int  g_houseList[8]; static int g_houseCount = 1;
static int  g_rotIdx = 0;
static int  g_house  = 2;      // currently displayed house
static void parseHouses() {
  g_houseCount = 0;
  const char* p = g_housesStr;
  while (*p && g_houseCount < 8) {
    while (*p == ' ' || *p == ',') p++;
    if (!*p) break;
    int v = atoi(p);
    if (v >= 0 && v < HMAX) g_houseList[g_houseCount++] = v;
    while (*p && *p != ',') p++;
  }
  if (g_houseCount == 0) { g_houseList[0] = 2; g_houseCount = 1; }
  if (g_rotIdx >= g_houseCount) g_rotIdx = 0;
  g_house = g_houseList[g_rotIdx];
}

// ---- 60-min trend, per house ------------------------------------------------
static constexpr int TR_N = 90;         // 90 samples x 40 s = 60 min
static float trendBuf[HMAX][TR_N];
static int   trendN[HMAX] = {0};
static void trendPush(int house, float v) {
  int& n = trendN[house];
  if (n < TR_N) trendBuf[house][n++] = v;
  else { memmove(trendBuf[house], trendBuf[house] + 1, (TR_N - 1) * sizeof(float)); trendBuf[house][TR_N - 1] = v; }
}
...
static void markAllDirty() {
  dTemp=dHumid=dCo2=dHd=dPres=dCur=dFlow=dW1=dW2=dTrend=true;
}
...
static uint32_t tempColor(float t) {
  ...
  for (int i = 1; i < N; i++) if (t <= ST[i]) {
    ...
    return display.color888(RR[i-1] + f*(RR[i]-RR[i-1]), GG[i-1] + f*(GG[i]-GG[i-1]), BB[i-1] + f*(BB[i]-BB[i-1]));
  }
  return display.color888(RR[N-1], GG[N-1], BB[N-1]);
}

// ---- heartbeat (freeze indicator) -------------------------------------------
...
static void drawHeartbeat() {
  ...
  display.fillRect(cx - r - 2, cy - r - 2, 2*r + 4, 2*r + 4, C_PANEL);
...
static void drawHeader() {
  ...
  char hs[36];
  if (g_houseCount > 1) snprintf(hs, sizeof(hs), "HOUSE %02d  [%d/%d]", g_house, g_rotIdx + 1, g_houseCount);
  else                  snprintf(hs, sizeof(hs), "HOUSE %02d", g_house);
...
static void drawHero() {
  ...
  if (!isnan(t)) {
    ...
    uint32_t ac = g_sev == SEV_NORMAL ? C_OK : sevMain();
    sprHero.fillArc(cx, cy, rO, rI, 135, 135 + frac * 270.0f, ac);
  }
  char buf[8]; if (isnan(t)) snprintf(buf, sizeof(buf), "--.-"); else snprintf(buf, sizeof(buf), "%.1f", t);
  ...
  sprHero.setTextColor(C_DIM);
  char h2[24]; snprintf(h2, sizeof(h2), "HOUSE %02d  別棟", g_house);
  sprHero.drawString(h2, 20, HERO_H - 74);
...
// ---- trend: room-temp 60 min (current house) --------------------------------
static void drawTrend() {
  float* buf = trendBuf[g_house];
  int    cnt = trendN[g_house];
  ...
  for (int i = 0; i < cnt; i++) { mn = min(mn, buf[i]); mx = max(mx, buf[i]); }
  ...
  auto X = [&](int i){ return cnt == 1 ? px : px + (int)((float)i / (cnt - 1) * pw); };
  ...
  int base = py + ph;
  for (int i = 1; i < cnt; i++) {
    int x0 = X(i - 1), x1 = X(i), y0 = Y(buf[i - 1]), y1 = Y(buf[i]);
    uint32_t col = tempColor((buf[i - 1] + buf[i]) * 0.5f);
    sprTrend.fillTriangle(x0, y0, x1, y1, x0, base, col);
    sprTrend.fillTriangle(x1, y1, x1, base, x0, base, col);
  }
  for (int i = 1; i < cnt; i++)
    sprTrend.drawLine(X(i - 1), Y(buf[i - 1]), X(i), Y(buf[i]), C_TEXT);
  int ex = X(cnt - 1), ey = Y(buf[cnt - 1]);
...
static void drawTileInto(M5Canvas& spr, int x, int y, int w, int h,
                         const char* label, float val, int dec, const char* unit, int valSize) {
  spr.fillSprite(C_BG);
  ...
  char buf[8];
  if (isnan(val)) snprintf(buf, sizeof(buf), dec ? "--.-" : "---");
  else            snprintf(buf, sizeof(buf), dec ? "%.1f" : "%.0f", val);
  spr.setFont(&fonts::Font7);
  spr.setTextSize(valSize);
  spr.setTextColor(C_TEXT);
  if (valSize > 1) { spr.setTextDatum(textdatum_t::middle_center); spr.drawString(buf, w / 2, h / 2 + 24); }
  else             { spr.setTextDatum(textdatum_t::bottom_right);  spr.drawString(buf, w - 92, h - 12); }
  spr.setTextSize(1);
...
// ---- window aperture bar ----------------------------------------------------
static void drawWindow(int idx) {
  HouseData& h = HS[g_house];
  int pct = idx == 0 ? h.w1p : h.w2p;
  int tgt = idx == 0 ? h.w1t : h.w2t;
  const char* src = idx == 0 ? h.w1s : h.w2s;
  int x = (idx == 0) ? MX : 644;
  ...
  char sub[48]; snprintf(sub, sizeof(sub), "target %d%%   src %s", tgt, src);
...
static void redrawDirty() {
  HouseData& h = HS[g_house];
  if (dTemp) {
    Sev ns = computeSev(h.temp);
    if (ns != g_sev) { g_sev = ns; drawHeader(); markAllDirty(); }
    drawHero(); dTemp = false;
  }
  if (dTrend) { drawTrend(); dTrend = false; }
  if (dCo2)   { drawTileInto(sprBig, CO2_X, BT_Y, BT_W, BT_H, "CO2",  h.co2,  0, "ppm",   2); dCo2=false; }
  if (dHd)    { drawTileInto(sprBig, HD_X,  BT_Y, BT_W, BT_H, "飽差", h.hd,  1, "g/m³", 2); dHd=false; }
  if (dHumid) { drawTileInto(sprSm, RC_X, 66,  RC_W, RC_H, "湿度", h.humid, 0, "%RH",  1); dHumid=false; }
  if (dPres)  { drawTileInto(sprSm, RC_X, 194, RC_W, RC_H, "気圧", h.pres,  0, "hPa",  1); dPres=false; }
  if (dCur)   { drawTileInto(sprSm, RC_X, 322, RC_W, RC_H, "電流", h.cur,   1, "A",    1); dCur=false; }
  if (dFlow)  { drawTileInto(sprSm, RC_X, 450, RC_W, RC_H, "灌水", h.flow,  1, "L/min",1); dFlow=false; }
  if (dW1)    { drawWindow(0); dW1=false; }
  if (dW2)    { drawWindow(1); dW2=false; }
}

static void switchHouse() {           // called on rotation tick / config change
  g_sev = computeSev(HS[g_house].temp);
  drawHeader();
  markAllDirty();
}
...
static void onMqtt(char* topic, byte* payload, unsigned int len) {
  ...
  if      (ends(topic, "/sensor/InAirTemp"))     { h.temp  = doc["value"] | NAN; if (cur) dTemp = true; }
  else if (ends(topic, "/sensor/InAirHumid"))    { h.humid = doc["value"] | NAN; if (cur) dHumid = true; }
  else if (ends(topic, "/sensor/InAirCO2"))      { h.co2   = doc["value"] | NAN; if (cur) dCo2 = true; }
  else if (ends(topic, "/sensor/InAirHD"))       { h.hd    = doc["value"] | NAN; if (cur) dHd = true; }
  else if (ends(topic, "/sensor/InAirPressure")) { h.pres  = doc["value"] | NAN; if (cur) dPres = true; }
  else if (ends(topic, "/sensor/Current"))       { h.cur   = doc["current_a"] | (doc["value"] | NAN); if (cur) dCur = true; }
...
static void mqttConnect() {
  ...
  String hs(mqttHost);
  if (hs.endsWith(".local")) {
    String name = hs.substring(0, hs.length() - 6);
...
static void loadConfig() {
  ...
  String hs  = prefs.getString("houses", "2");
  strlcpy(g_housesStr, hs.c_str(), sizeof(g_housesStr));
  g_rotSecs  = prefs.getInt("rotsec", 15);
...
static void saveConfig() {
  ...
  prefs.putInt("skin", g_skin);
  prefs.putString("houses", g_housesStr);
...
static String htmlPage() {
  ...
  s += FW_NAME; s += " v"; s += FW_VERSION; s += " · "; s += WiFi.localIP().toString();
  snprintf(b,sizeof(b)," · 表示中 HOUSE %02d</small>",g_house); s += b;
  HouseData& h = HS[g_house];
  s += F("<h2>現在値</h2><div class=row>");
  snprintf(b,sizeof(b),"<div>室温<div class=v>%.1f&deg;C</div></div>",h.temp); s+=b;
  snprintf(b,sizeof(b),"<div>湿度<div class=v>%.0f%%</div></div>",h.humid); s+=b;
...
static void handleStatus() {
  ...
  d["skin"]=g_skin; d["houses"]=g_housesStr; d["house"]=g_house; d["rotsec"]=g_rotSecs;
  HouseData& hh = HS[g_house];                 // omit NaN so the JSON stays valid
...
static void handleSave() {
  ...
  if (server.hasArg("houses")) strlcpy(g_housesStr, server.arg("houses").c_str(), sizeof(g_housesStr));
  if (server.hasArg("rotsec")) g_rotSecs = server.arg("rotsec").toInt();
...
void loop() {
  ...
  // rotate the displayed house (only when more than one configured)
  if (g_houseCount > 1 && now - lastRot > (uint32_t)g_rotSecs * 1000) {
    lastRot = now;
    g_rotIdx = (g_rotIdx + 1) % g_houseCount;
    g_house = g_houseList[g_rotIdx];
    switchHouse();
  }

  // sample every configured house's room temp into its own trend (40 s)
  if (lastSample == 0 ? now > 2000 : now - lastSample > 40000) {
    lastSample = now;
    for (int i = 0; i < g_houseCount; i++) {
      int hh = g_houseList[i];
      if (!isnan(HS[hh].temp)) { trendPush(hh, HS[hh].temp); if (hh == g_house) dTrend = true; }
    }
  }
